...
chrom_lim <- read.delim(file="chr.ends.new.popseq", header = F, sep = "\t")
...
neipiwind101d <- read.delim(file="neipi_window_domest_57samples_var_nonvar.popseq17.txt", header = F, sep = "\t")
neipiwind101w <- read.delim(file="neipi_window_wild_302samples_var_nonvar.popseq17.txt", header = F, sep = "\t")
...
neipi_gene_d <- read.table("NeiPi.bygene.57samples_domest_homo_allconf_UG.miss0.5.counts.mapping.newpopseq", sep="\t", header = F)
neipi_gene_w <- read.table("NeiPi.bygene.302samples_wild_homo_allconf_UG.miss0.5.counts.mapping.newpopseq", sep="\t", header = F)
...
faywuwind101 <- read.delim(file="faywu_window_size10_step1_wild_dom_var_nonvar.popseq17.txt", header = F, sep = "\t")
...
faywu_genes_all <- read.delim(file="faywu.bygene.outgroup.nohead.478sampl.filtered.SNP.478samples.UG.genotyping.minN1.DP8.pcrerr5e-2.homo.with_sing.all.counts.cutoff5incl.newpopseq", header = F, sep = "\t")
...
fst_genes <- read.delim(file="fst.bygene.478sampl.filtered.SNP.478samples.UG.genotyping.minN1.DP8.pcrerr5e-2.homo.with_sing.all.counts.position", header = F, sep = "\t")
...
sweed_genes <- read.delim(file="57samples_domest.sweed.grid5.snp3.newpopseq", header = F, sep = "\t")

sweed_geneswild <- read.delim(file="302samples_wild.sweed.grid2.snp3.newpopseq", header = F, sep = "\t")
...
plot_circ <- function(){ggplot() +
	geom_line(data = circ_chrom, aes(y = -.1, x = pos, group=chr), 
		size = .5, colour = "black") +
	geom_line(data = circ_chrom, aes(x=pos+1, y=1.1, group=chr), 
		alpha=0.5) +
	geom_text(data=chr_label,aes(x=V1,y=-.2,label=chr), 
		size=3) + 
	geom_point(data = nonoutliers_nei_gene, aes(x=pos_circ,y=ratio_sc), 
		colour=c("darkgrey"), size = .2, alpha = .6) + # gene nonoutlier dots
	geom_point(data = outliers_nei_gene, aes(x=pos_circ,y=ratio_sc), 
		colour=c("#8254f2"),  size = .5, alpha = 1) + # gene outlier dots
	geom_line(data = neipiwind101_wide, aes(x=pos_circ, y=ratio_sc, group=chr), 
		colour=c("#8254f2"), size=.5, alpha=.8) + #shape=19, 
	geom_segment(data=ticks,aes(x=ticks,xend=ticks,y=1.1,yend=1.14), 
		alpha=.5) +
	geom_text(data = ticks_mod, aes(x=ticks, y=1.2, label=label), 
		size = 2, alpha = .7) +
	geom_segment(aes(x = 0,xend = max(circ_chrom$pos), 
		y = nei_gene_thresh, yend = nei_gene_thresh), linetype="dashed", size=.1) +
	geom_point(data = circular(alloutplot), aes(x = pos_circ, y = 1.35), 
		size = 2, colour=c("brown"), alpha=0.7) +
	geom_segment(aes(x=1,xend=1,y=0,yend=1.01), 
		size = 0.2, alpha =.5, linetype="dashed") + # vertical line
	geom_segment(data = plotneiscale, aes(x=0,xend=1, y=neiscale_sc, yend = neiscale_sc), 
		size= .4, alpha = .5) + # ticks
	geom_text(data = plotneiscale, aes(x=0,y=neiscale_sc,label=text), 
		size=1, alpha=.7, hjust=1) +
	geom_segment(data = plotneiscale_genes, aes(x=0, xend=1, y=neiscale_sc, yend = neiscale_sc), 
		size= .4, alpha = .5) + # ticks
	geom_text(data = plotneiscale_genes, aes(x=0,y=neiscale_sc,label=text), 
		size=1, alpha=.7, hjust=1) + # text
	geom_point(data = nonoutliers_sweed_genes, aes(x=pos_circ,y=val_sc), 
		colour=c("darkgrey"), size = .2, alpha = .6) + # gene nonoutlier dots
	geom_point(data = outliers_sweed_genes, aes(x=pos_circ,y=val_sc), 
		colour=c("red"),  size = .5, alpha = 1) + # gene outlier dots
	geom_segment(aes(x = 0,xend = max(circ_chrom$pos), 
		y = sweed_genes_thresh, yend = sweed_genes_thresh), linetype="dashed", size=.1) +
	geom_segment(data = plotswscale_genes, aes(x=0,xend=1, y=swscale_sc, yend = swscale_sc),
		size= .4, alpha = .5) + # ticks
	geom_text(data=plotswscale_genes, aes(x=0,y=swscale_sc,label=text), 
		size=1, alpha=.7, hjust=1, angle = 0) + # text
	coord_polar(theta = "x") +
	...
	geom_point(data = faywu_gene_outliers, aes(x=pos_circ,y=val_sc), 
		colour = "#d95f02", size = .5, alpha = .8) + # gene outlier dots
	geom_point(data = faywu_gene_rest, aes(x=pos_circ,y=val_sc), 
		colour="grey", size = .5, alpha = .5) + # gene outlier dots
	geom_line(data=subset(faywuwind101, type %in% "domest"), aes(x=pos_circ, y=val_sc, group=chr), 
		colour=c("#d95f02"), size=.5, alpha=.7) +
	geom_line(data=subset(faywuwind101, type %in% "wild"), aes(x=pos_circ, y=val_sc, group=chr), 
		colour=c("#1b9e77"), size=.5, alpha= .7) +
	geom_segment(aes(x = 0,xend = max(circ_chrom$pos), y = faywu_genes_thresh, yend = faywu_genes_thresh), 
		linetype="dashed", size=.1) +
	geom_segment(data = plotfayscale, aes(x=0,xend=1, y=fayscale_sc, yend = fayscale_sc), 
		size= .4, alpha = .5) + # ticks
	geom_text(data=plotfayscale, aes(x=0,y=fayscale_sc,label=fayscale), 
		size=1, alpha=.7, hjust=1, angle = 0) + # text
...
genes <- read.delim("selection_genes", header=F, sep="\t")
...
allout[is.na(allout)]<-""
write.table(allout,file="outliers.sweed_fst_fay_nei.out", quote=F, sep="\t", row.names = F)
